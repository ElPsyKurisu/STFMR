...
if len(sys.argv) == 1: directories = ["BPBO-LSMO-LSAT_0122622a\\dev21"]
else: directories = sys.argv[1:] 

num_dir = len(directories)
freq_dir = [] #nested list of all the frequencies to plot in a given directory
...
for i in range(num_dir):
    ...
    for a in range(number_of_files): #build freq_dir
            freq_dir.append(files[a]) 
    for j in range(number_of_files): #open each file in freq_dir
